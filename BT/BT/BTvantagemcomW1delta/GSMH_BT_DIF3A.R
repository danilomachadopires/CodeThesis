...
# GW irá determinar em que partida o jogador participou, pois cada linha corresponde a uma jogo e cada coluna
# a um jogador, assim se o jogador 3 ("AlmasiZoltan") tiver jogado a partida 1000, então na posição GW[1000,3]
# teŕa o número 1 
for(i in 1:m){
...
M <- GW+GB
fora <- which(apply(M,1,sum)==0)  # posições referentes a partidas  em que nem dos jogadores analisados participou
n-length(fora)
...
### chute inicial
## esse chute inicial corresponde aos valores iniciais atribuidos aos parametros de rating dos jogadores analisados
## e ao delta 
...
# Os ratings usados como chute inicial dos 46 jogadores são dados por 2705 e para os demais foi usados os ratings
# ja tinham.
...
while (cont <= nsMC) {
  # Atualizar o gama
  for(i in sample(1:m)){
    indice <- which((GW[,i]+GB[,i])!=0)  # posição em que o jogador i jogou (partida que ele jogou)
    # Construir a matriz Theta para a partidas em que o jogador esteve presente
    # assim é separado os ratings dos jogadores envolvidos em cada uma das partidas do indice i
    Theta  <- NULL
    ...
    # Valor corrente da log-Verossimilhança
     Y<-y[indice]
    omega<-omegag[indice]
    # a verossimilhança para cada jogador, ou seja, com todos os valores de pi (calculados) e todos os valores
    # de Y observados é calculado uma verossimolhança (soma de todas as verossimilhanças)
    lVa <- lLpi(pi) 
    # Amostra candidato
    cand <- rnorm(1,mugC[i],sigmagC[i])
    
    # Valor da log-Verossimilhança para o candidato
    # é atribuido o rating candidado para o jogador analisado
