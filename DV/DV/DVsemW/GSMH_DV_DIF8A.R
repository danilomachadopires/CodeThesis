...
rm(list=ls(all=T))
source("/home/danilomp/Qualifica/DV/DVsemW/FGSMH.R")

dados <- read.table("/home/danilomp/Qualifica/DV/DVsemW/granprix2010a2013.txt",h=T)
attach(dados)
...
load("/home/danilomp/Qualifica/DV/DVsemW/cgr.rda")
...
while (cont <= nsMC) {
  ...
  if(cont%%J==0 && cont>B){
    write(t(gama),"/home/danilomp/Qualifica/DV/DVsemW/cadeia.gamaA.txt",ncol=length(gama),append=TRUE)
    write(lambda,"/home/danilomp/Qualifica/DV/DVsemW/cadeia.lambdaA.txt",ncol=1,append=TRUE)
    # Valor corrente da logVerossimilhança
    # para o cálculo do fator de Bayes
    pi  <- DV.Dif(WR,BR,lambda)
    lVm <- lLpi3(pi)
    write(lVm,"/home/danilomp/Qualifica/DV/DVsemW/cadeia.lVmA.txt",ncol=1,append=TRUE)
  }
