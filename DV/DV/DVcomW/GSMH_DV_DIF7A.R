...
rm(list=ls(all=T))
source("/home/danilomp/Qualifica/DV/DVcomW/FGSMH.R")

dados <- read.table("/home/danilomp/Qualifica/DV/DVcomW/granprix2010a2013.txt",h=T)
attach(dados)
...
load("/home/danilomp/Qualifica/DV/DVcomW/cgr.rda")
...
rm(cgr)
cgr <- cbind(c(mugC,mulgC),c(sigmagC,sdlgC))
...
Mtcgc <- matrix(0,(m+1),tcgc) # ultimas amostras
pi    <- DV.Dif(WR,BR,lambda) 
Y<-Yg
omega <- omegag
...
while (cont <= nsMC) {
  ...
  for(i in ordem){
    ...
    gamaic   <- WR[indice]
    gamajc   <- BR[indice]
    pi <- DV.Dif(gamaic,gamajc,lambda)
  ...
  if(cont%%J==0 && cont>B){
    write(t(gama),"/home/danilomp/Qualifica/DV/DVcomW/cadeia.gamaA.txt",ncol=length(gama),append=TRUE)
    write(lambda,"/home/danilomp/Qualifica/DV/DVcomW/cadeia.lambdaA.txt",ncol=1,append=TRUE)
    # Valor corrente da logVerossimilhança
    # para o cálculo do fator de Bayes
    pi  <- DV.Dif(WR,BR,lambda)
    lVm <- lLpi3(pi)
    write(lVm,"/home/danilomp/Qualifica/DV/DVcomW/cadeia.lVmA.txt",ncol=1,append=TRUE)
  }
  # atualizar geradora de candidatos
  Mtcgc[,cont%%tcgc] <- c(gama,lambda)
  if(cont%%tcgc==0){
