...
apply(rd.gamaA,2,max);apply(rd.lambdaA,2,max)
apply(rd.gamaB,2,max);apply(rd.lambdaB,2,max)
apply(rd.gamaC,2,max);apply(rd.lambdaC,2,max)
...
#HPDinterval(cadeia.gama)
#dados<-data.frame(nomes=estimandos,gama=res.gama[1]$statistics[,1],
#                   ic.inf=HPDinterval(cadeia.gama)[,1],
#                   ic.sup=HPDinterval(cadeia.gama)[,2])
